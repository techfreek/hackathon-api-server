...
function getImageNames(req, res) {
  function measureImage(path, year, callback) {
    var filepath = path.replace(config.imagesDir, "hosted_images");
    ...
    calipers.measure(path, sendImage.bind(null, img, calipers));
  }

  function sendImage(img, callback, err, size) {
    if(err) {
      callback(err);
    } else {
      img.width = size.pages[0].width;
      img.height = size.pages[0].height;
      callback(null, img);
    }
  }
  ...
  glob(img_path, function(err, names) {
    ...
    for(var i = 0; i < chunk.length; i++) {
      tasks.push(async.apply(measureImage, path, year));
    }

    async.parallel(function(err, imgs) {
      res.send({
        imgs: imgs,
